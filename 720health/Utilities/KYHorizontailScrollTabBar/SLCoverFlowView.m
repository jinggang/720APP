...
static const CGFloat SLCoverWidth = 50.0;
static const CGFloat SLCoverHeight = 50.0;
...
@property (nonatomic, strong) NSMutableArray *allCoverView;
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _allCoverView = [NSMutableArray array];
    }
    return self;
}
...
- (SLCoverViewWrapper *)addNewCoverViewOnRight:(CGFloat)rightEdge index:(NSInteger)index {
    ...
    if (index >= 0 && index < self.parentView.numberOfCoverViews) {
        ...
        [_allCoverView addObject:coverView];
    }
    return wrapper;
}
...
- (SLCoverViewWrapper *)addNewCoverViewOnLeft:(CGFloat)leftEdge index:(NSInteger)index {
    ...
    if (index >= 0 && index < self.parentView.numberOfCoverViews) {
        ...
        [_allCoverView insertObject:coverView atIndex:0];
    }
    return wrapper;
}
...
- (void)setCoverSize:(CGSize)coverSize {
    if (!CGSizeEqualToSize(_coverSize, coverSize)) {
        ...
        NSLog(@"centetIndex===%d",centerIndex);
...
- (void)reloadData {
    ...
    _coverViews = _scrollView.allCoverView;
